package by.ibrel.kitan.constants;

/**
 * @author ibrel
 * @version 1.0 (05/12/16)
 */
public final class UrlConstants {

    public final static String REGISTRATION_URL = "/registration";
    public final static String LOGIN_EXISTS_CHECK = "/checkLogin";

    public final static String LOGIN_URL = "/login_";
    public final static String HOME_URL = "/home";
    public final static String CONTACT_URL = "/contacts";
    public final static String HELP_URL = "/help";
    public final static String REF_URL = "/ref";
    public final static String ABOUT_URL = "/about";
    public final static String USER_LIST_URL = "/admin//user/list";
    public final static String USER_EDIT_URL = "/user/edit";
    public final static String ROLE_LIST_URL = "/admin/role/list";
    public final static String CLIENT_LIST_URL = "/client/list";
    public final static String CLIENT_ADD_URL = "/client/add-page";
    public final static String CART_LIST_URL = "/cart/list";
    public final static String PRODUCT_LIST_URL = "/product/list";
    public final static String PRODUCT_ADD_URL = "/product/add-page";
    public final static String CART_SHOW_URL = "/cart/show";
    public final static String PRICE_LIST_URL = "/configuration/price/list";
    public final static String PRICE_ADD_URL = "/configuration/price/add-page";
    public final static String PRODUCT_CATEGORY_LIST_URL = "/configuration/product-category/list";
    public final static String PRODUCT_COLOR_LIST_URL = "/configuration/product-color/list";
    public final static String ERROR_404_URL = "/error/404";
    public final static String ERROR_405_URL = "/error/405";
    public final static String ERROR_TRACE_URL = "/error/trace";
    public final static String ERROR_403_URL = "/error/403";
}
